...
SITE = "https://cyclonerake.com"
START_URLS = [f"{SITE}/", f"{SITE}/docs/"]
ROBOTS_URL = f"{SITE}/robots.txt"
SITEMAP_GUESSES = [
    f"{SITE}/sitemap.xml",
    f"{SITE}/sitemap_index.xml",
    f"{SITE}/sitemap-en.xml",
]

STATE_PATH = os.path.join(os.path.dirname(__file__), "..", "state_cyclonerake.json")
...
# =========================
# Azure Identity / Clients
# =========================

def get_azure_credential():
    """Match infra behavior: use MI if USE_MI_AUTH=true, else DefaultAzureCredential (no interactive)."""
...
# Ensure index contains a 'url' field (non-key, string)
...
ASSET_EXT = {
    ".png",".jpg",".jpeg",".gif",".webp",".svg",".pdf",".zip",".gz",".mp4",".mov",
    ".css",".js",".ico",".woff",".woff2",".ttf",".eot",".otf",".xml",".json"
}
...
def norm_url(u: str) -> str:
    ...
    path = p.path or "/"
    # normalize: strip query/fragment; remove trailing slash except root
...
def render_html(url: str, wait_selector: str = "body", timeout_ms: int = 15000) -> Optional[str]:
    """Render URL with headless Chromium and return full HTML (incl. JS content)."""
    try:
        br = _Browser()
        p = br.page()
...
def load_robots() -> Tuple[Optional[robotparser.RobotFileParser], List[str]]:
    """Fetch and parse robots.txt; return (RobotFileParser or None, list of sitemap URLs)."""
    try:
        r = requests.get(ROBOTS_URL, headers={"User-Agent": UA}, timeout=15)
        dbg("robots fetch:", r.status_code, r.headers.get("content-type"))
        if not r.ok:
            return None, []
        ...
        if sitemaps:
            dbg("robots sitemaps:", len(sitemaps))
        return rp, sitemaps
    except Exception as e:
        dbg("robots error:", type(e).__name__, str(e))
        return None, []

def parse_sitemap(sitemap_url: str) -> Tuple[List[str], List[str]]:
    """Return (page_urls, child_sitemaps) from a sitemap or sitemap index."""
    try:
        ...
        dbg("sitemap parse error:", sitemap_url, type(e).__name__, str(e))
        return [], []

def allowed_by_robots(rp: Optional[robotparser.RobotFileParser], url: str) -> bool:
...
def discover_urls(max_pages: int = 1000, sleep_sec: float = 0.15) -> List[str]:
    """Discover URLs site-wide, honoring robots.txt if present. Prefer sitemaps; fallback to BFS from START_URLS."""
    seen: Set[str] = set()
    rp, robot_sitemaps = load_robots()

    # 1) Try sitemaps from robots.txt, then common guesses
    to_visit_sm: List[str] = []
    if robot_sitemaps:
        to_visit_sm.extend(robot_sitemaps)
    else:
        to_visit_sm.extend(SITEMAP_GUESSES)

    visited_sm: Set[str] = set()
    while to_visit_sm and len(seen) < max_pages:
        sm = to_visit_sm.pop(0)
        if sm in visited_sm:
            continue
        visited_sm.add(sm)
        dbg("sitemap fetch:", sm)
        url_list, child_sms = parse_sitemap(sm)
        for href in url_list:
            nu = norm_url(href)
            if is_allowed_url(nu) and allowed_by_robots(rp, nu):
                seen.add(nu)
                if len(seen) >= max_pages:
                    break
        for child in child_sms:
            if child not in visited_sm:
                to_visit_sm.append(child)

    if seen:
        urls = sorted(seen)
        return urls[:max_pages]

    # 2) Fallback: BFS crawl starting from seeds
    q: List[str] = list(START_URLS)
    while q and len(seen) < max_pages:
        url = q.pop(0)
        if url in seen or not is_allowed_url(url) or not allowed_by_robots(rp, url):
            continue
        seen.add(url)
        try:
            if USE_BROWSER:
            ...
            for a in soup.select("a[href]"):
                ...
                if is_allowed_url(nu) and allowed_by_robots(rp, nu) and nu not in seen and nu not in q:
                    q.append(nu)
                    links += 1
...
# Strip UI fluff; focus on content
RM_SELECTORS = [
...
def sections_from_dom(main: BeautifulSoup) -> List[Tuple[str, str]]:
    """Split by headings (h2/h3) to get semantically coherent chunks before token chunking."""
...
def run(max_pages: int = 500, sleep_sec: float = 0.2):
        ...
        for heading, body in secs:
            ...
            for idx, (c, v) in enumerate(zip(chunks, vecs)):
                ...
                changed_docs.append({
                    ...
                    "chunk": c,
                    "text_vector": v,
                    "url": url,  # <-- keep source URL for UI display
                })
            section_map[section_key] = body_hash
    ...
    dbg("done")
    print(f"[cyclonerake] pages: {len(urls)}, new/updated chunks: {total_chunks}, upserts: {total_upsert}")
